...
1. **光合作用收入（Income）**
   ...
   - `Income = Sunlight * (0.04 + Gene * 0.0056)`
   - *（注：目前版本暂时移除了地形肥力设定，所有格子均能公平获取光照。）*

2. **基础代谢支出（Cost）**
   维持生命的基础开销，基因越激进，代谢支出越高（呈平方级增长）。
   - 参数化写法：`Cost0 = baseCost + Gene² × geneCostFactor`
   - 默认参数：`baseCost = 0.0004`，`geneCostFactor = 0.002`，所以等价于 `Cost0 = 0.0004 + Gene² × 0.002`

3. **能量扩散（Diffusion）**

   能量扩散是“守恒的局部流动”，但不是无脑平均。想要发生能量外流，必须**同时满足三个条件**：
   1. **自身有余粮**：自身的能量必须 > 0（负能量不会向外扩散）。
   2. **有活体邻居**：8 邻域内至少有一个“活体植物”（`Type = PLANT` 且 `Biomass > 0`）。空地和墙体**不接收**扩散，也不参与邻居平均值计算。
